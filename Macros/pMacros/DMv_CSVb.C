...
TChain TT("bdttree");
TT.Add("/lstore/cms/cbeiraod/Stop4Body/nTuples_v2017-10-19_test_skimmed/Wjets_200to400_skimmed.root");
...
TH1F *tt = new TH1F("tt","Wjets;Discriminant(highest b-tag jet)",Xbin,Xmin,Xmax);
TT.Draw("JetHBCSV>>tt",presel);
double ttn = tt->GetEntries();
tt->Scale(1./ttn);
tt->SetFillColor(kGray);
//tt->SetLineWidth(2);
TH1F *dm1 = new TH1F("dm1","Signal=(300,290);Discriminant(highest b-tag jet)",Xbin,Xmin,Xmax);
DM1.Draw("JetHBCSV>>dm1",presel);
double tn = dm1->GetEntries();
dm1->Scale(1./tn);
dm1->SetLineColor(kRed);
dm1->SetLineWidth(2);
TH1F *dm2 = new TH1F("dm2","Signal=(300,270);Discriminant(highest b-tag jet)",Xbin,Xmin,Xmax);
DM2.Draw("JetHBCSV>>dm2",presel);
double sn = dm2->GetEntries();
dm2->Scale(1./sn);
dm2->SetLineColor(kRed+1);
dm2->SetLineWidth(2);
TH1F *dm3 = new TH1F("dm3","Signal=(300,250);Discriminant(highest b-tag jet)",Xbin,Xmin,Xmax);
DM3.Draw("JetHBCSV>>dm3",presel);
double w1n = dm3->GetEntries();
dm3->Scale(1./w1n);
dm3->SetLineColor(kRed+3);
dm3->SetLineWidth(2);
TH1F *dm4 = new TH1F("dm4","Signal=(300,220);Discriminant(highest b-tag jet)",Xbin,Xmin,Xmax);
DM4.Draw("JetHBCSV>>dm4",presel);
