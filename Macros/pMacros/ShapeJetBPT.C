...
TH1F *ht = new TH1F("ht","pT(highest b jet)",Xbin,Xmin,Xmax);
TT.Draw("JetHBpt>>ht");
double tn = ht->GetEntries();
ht->Scale(1./tn);
ht->SetLineColor(1);
ht->SetLineWidth(2);
TH1F *hs = new TH1F("hs","pT(highest b jet)",Xbin,Xmin,Xmax);
S1.Draw("JetHBpt>>hs");
double sn = hs->GetEntries();
hs->Scale(1./sn);
hs->SetLineColor(2);
hs->SetLineWidth(2);
TH1F *hw1 = new TH1F("hw1","pT(highest b jet)",Xbin,Xmin,Xmax);
W1.Draw("JetHBpt>>hw1");
double w1n = hw1->GetEntries();
hw1->Scale(1./w1n);
hw1->SetLineColor(3);
hw1->SetLineWidth(2);
TH1F *hw2 = new TH1F("hw2","pT(highest b jet)",Xbin,Xmin,Xmax);
W2.Draw("JetHBpt>>hw2");
double w2n = hw2->GetEntries();
hw2->Scale(1./w2n);
hw2->SetLineColor(4);
hw2->SetLineWidth(2);
TH1F *hw3 = new TH1F("hw3","pT(highest b jet)",Xbin,Xmin,Xmax);
W3.Draw("JetHBpt>>hw3");
double w3n = hw3->GetEntries();
hw3->Scale(1./w3n);
hw3->SetLineColor(5);
hw3->SetLineWidth(2);
TH1F *hw4 = new TH1F("hw4","pT(highest b jet)",Xbin,Xmin,Xmax);
W4.Draw("JetHBpt>>hw4");
...
hw4->Draw("same");
 ...
 lg->AddEntry(hw4,"Wjets 600<HT","l");
 lg->AddEntry(hs,"Signal (300,270)","l");
 lg->Draw();
 c1->SaveAs("ShapeJetBPT_300-270.pdf");
