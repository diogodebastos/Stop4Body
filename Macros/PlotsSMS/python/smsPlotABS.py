...
class smsPlotABS(object):
    ...
    def DrawText(self):
        ...
        if getattr( self.model, 'signifPlot', False):
            pass
        ...
        # MODEL LABEL
        dx = 0.01 if hasattr(self.model, 'signifPlot') else 0
        if(self.model.label2 == ""):
            textModelLabel= rt.TLatex(0.16+dx,0.90,"%s" %self.model.label)
            ...
            textModelLabel.Draw()
            self.c.textModelLabel = textModelLabel
            textModelLabel2= rt.TLatex(0.16,0.845,"%s    NLO+NLL exclusion" %self.model.label2)
        ...
        extraTexts = getattr(self.model, "extraTexts", [])
        print extraTexts
        if extraTexts:
            self.c.textExtras = []
            for extraTextDict in extraTexts:
                print extraTextDict
                text      = extraTextDict['text']
                textAlign = extraTextDict.get('align', 13) 
                textFont  = extraTextDict.get('font', 42) 
                textSize  = extraTextDict.get('size', 0.036) 
                textPos   = extraTextDict.get('position', [0.45,0.25] )
                textExtra = rt.TLatex( textPos[0], textPos[1], text)
                textExtra.SetNDC()
                textExtra.SetTextAlign( textAlign )
                textExtra.SetTextFont( textFont )
                textExtra.SetTextSize( textSize )
                textExtra.Draw()
                self.c.textExtras.append( textExtra )
        if getattr(self.model, "extraText",""):
            #if self.dmplot:
            #    chipmpos = [0.48,0.24]
            #else:
            #    chipmpos = [0.48,0.24]
            chipmpos = getattr( self.model, 'extraTextPos', [0.48,0.24] )
            textExtra = rt.TLatex(chipmpos[0],chipmpos[1], self.model.extraText)
            textExtra.SetNDC()
            textExtra.SetTextAlign(13)
            textExtra.SetTextFont(42)
            textExtra.SetTextSize( getattr( self.model, 'extraTextSize', 0.036) )
            textExtra.Draw()
